...
└─────────────┘     └──────────────┘     └───────────────┘
                  ...
                  │  Step 3: joarchy fb  │──▶ 兜底（by name）
                  │  Step 4: 合并        │──▶ api_results
                  │  Step 5: 偏移/冻结   │──▶ offset_kd, offset_kpm
                  └──────────────────────┘
                           │
           ┌───────────────┼───────────────┐
           ▼               ▼               ▼
   gametools.network  joarchy.com(UK)  joarchy.com(US)
```
...
推荐值1.313基于5组样本数据的平均偏差得出。

**偏移系数冻结机制**：当 API（gametools + joarchy）覆盖率达到 80% 以上时，偏移系数自动冻结（设为 None），未查到的玩家直接使用 Excel 原始数据。原因是高覆盖率下偏移样本太少，计算结果反而不准。
...
| 数据源 | 用途 | 优势 | 劣势 |
|---|---|---|---|
| gametools.network | KD 查询 + userId 提取 | 稳定，KD 准确 | KPM 含挂机时间，偏低 |
| joarchy.com (BF6 Tracker) | KPM 修正 + 兜底查询 | KPM 更准（去除非战斗时间），本地缓存快 | 偶尔 502，第三方项目 |
...
**identifier 机制**：EA 平台为每个玩家分配的永久数字 ID（platformUserIdentifier），不会因改名而变化。gametools 返回的 `userId` 字段即为 joarchy 的 `identifier`，可用于稳定查询。

**joarchy 主备容错**：主节点 `api-btr-joo-uk.joarchy.com`（UK）不可用时自动切换到备用节点 `api-btr-joo-us.joarchy.com`（US）。
...
| 页面 | 功能 | 控件 | 跳过条件 |
|---|---|---|---|
| 1. 导入 | 选择Excel，预览数据 | 文件选择、数据表格 | - |
| 2. API 查询 | 自动查询真实数据 | 进度条、转圈动画、实时统计 | - |
| 3. 分配模式 | 选择均衡/随机 | 两张大卡片（单选） | - |
| 4. 游戏模式 | 选择征服/突破 | 两张大卡片（单选） | 随机模式自动跳过 |
| 5. 自定义小队 | 绑定玩家对 | 下拉框选择、添加/删除 | - |
| 6. 结果 | 查看分队和分析 | 左右分栏表格、候补表格、告警 | - |
| 7. 历史记录 | 浏览过往分队记录 | 记录列表表格、详情区（左右分栏） | 未点击历史记录按钮时跳过 |
| 8. 操作 | 重新导入/分配/退出 | 三个大按钮 | - |

### 4.2 步骤指示器

- 7个圆形步骤指示器，对应7个主要步骤（历史页为中间页，无指示器）
- 已完成步骤：绿色
- 当前步骤：蓝色高亮
- 未来步骤：灰色
...
- 偏移系数推荐值基于5组样本，精度有限（高覆盖率时自动冻结）
- 贪心算法是局部最优，非全局最优解（均衡模式）
- 历史记录最多保存5条，自动淘汰最早的（详见 `history.py`）
- 打包体积较大（~200MB），因Anaconda环境包含冗余依赖
- joarchy.com 为第三方个人项目，偶有 502 错误，长期可用性不确定
- joarchy 的 KPM 口径与 gametools 不同（去除非战斗时间），两者不能直接对比
- 极少数玩家在 gametools 和 joarchy name 查询都查不到时，只能使用 Excel 原始数据
