...
Reads Excel file (4 columns: NAME, KD, KPM, EAID),
outputs players.json. Offset correction is handled
by api_query.py based on real API data.
"""
...
def extract_players(xlsx_path: str) -> list:
    """
    Read player data from Excel, return a list of player dicts.

    Excel format (4 columns, all required):
        Column A: 昵称 (nickname)
        Column B: KD (from XiaoHeiHe)
        Column C: KPM (from XiaoHeiHe)
        Column D: EAID (EA username for API lookup)

    Returns list of dicts with keys:
        name, kd_raw, kpm_raw, kd, kpm_adjusted, eaid
    """
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        # Parse EAID (optional - players without EAID will use Excel data + offset)
        eaid = str(row[3]).strip() if len(row) > 3 and row[3] is not None else ""

        # kpm_adjusted initially equals kpm_raw (no offset)
        # Will be updated after API query by api_query module
        players.append({
            "name": name,
            "kd_raw": kd,
            "kpm_raw": kpm,
...
def main():
    if len(sys.argv) < 2:
        print("用法: python extract.py <Excel文件路径>")
        print(f"示例: python extract.py players.xlsx")
        sys.exit(1)
    ...
    players = extract_players(xlsx_path)
